...
def handle_propose(body: dict) -> JSONResponse:
    ...
    for d in dossiers_raw:
        ...
        call_id = stable_call_id(content_fp)
        ...
        digest = proposal_digest(d["dossierId"], decision["action"], decision["target"], decision["payload"])
        store.save_proposal(
            call_id, d["dossierId"], decision["action"], decision["target"], decision["payload"], digest
        )
...
def _process_receipt(r: Receipt) -> dict:
    ...
    if r.proposalDigest and r.proposalDigest != proposal["digest"]:
        return _rejected(r, "proposalDigest does not match persisted proposal", proposal["action"])
